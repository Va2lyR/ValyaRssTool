...
try {
    ...
    Write-Host "ERROR: Failed to load .NET assemblies. $_" -ForegroundColor Red
    exit 1
}
...
function Invoke-ToolExecution {
    ...
    try {
        if ([string]::IsNullOrWhiteSpace($Url)) { throw "URL is missing for this tool." }
        $cleanUrl = $Url.Trim()
        if (-not ($cleanUrl -match '^https?://')) { throw "Invalid URL format." }
        if (-not ($cleanUrl -match '^https://raw\.githubusercontent\.com/')) {
            $result = [System.Windows.Forms.MessageBox]::Show("This tool is not from a trusted source (GitHub).`n`nURL: $cleanUrl`n`nContinue anyway?", "Security Warning", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Warning)
        ...
        $cmd = "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; `$Host.UI.RawUI.WindowTitle = 'ValyaRssTool - $Name'; Write-Host ''; Write-Host '============================================================' -ForegroundColor Cyan; Write-Host '  ValyaRssTool - Executing: $Name' -ForegroundColor Cyan; Write-Host '============================================================' -ForegroundColor Cyan; Write-Host '  Source: $cleanUrl' -ForegroundColor Gray; Write-Host '  Time:   `$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')' -ForegroundColor Gray; Write-Host '============================================================' -ForegroundColor Cyan; Write-Host ''; try { irm '$cleanUrl' -ErrorAction Stop | iex } catch { Write-Host 'ERROR: Failed to execute tool. `$_' -ForegroundColor Red; Write-Host 'Press any key to exit...' -ForegroundColor Yellow; [void]`$Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown') }"
        $si = New-Object System.Diagnostics.ProcessStartInfo
        $si.FileName = "powershell.exe"
        $si.Arguments = "-NoExit -ExecutionPolicy Bypass -Command `"$cmd`""
        $si.Verb = "runas"
        ...
        [System.Windows.Forms.MessageBox]::Show("Failed to launch tool:`n`n$($_.Exception.Message)", "Execution Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
...
function New-ToolCard {
    ...
    $btnRun.Add_Click({ $selected = $this.Tag; Invoke-ToolExecution -Url $selected.Url -Name $selected.Name })
    $card.Controls.Add($btnRun)
    $card.Add_MouseEnter({ $this.BackColor = $script:theme.CardHover; $script:statusLabel.Text = "Category: $($this.Tag.Category) | Double-click to run" })
    $card.Add_MouseLeave({ $this.BackColor = $script:theme.Card; $script:statusLabel.Text = "Hover over a tool to see details | Double-click card to run" })
    $card.Add_DoubleClick({ Invoke-ToolExecution -Url $this.Tag.Url -Name $this.Tag.Name })
...
function Render-Tools {
    [CmdletBinding()]
    param([string]$FilterText = "", [string]$CategoryFilter = "All Categories")
...
$txtSearch.Add_GotFocus({
    if ($this.Text -eq "Search tools...") { $this.Text = ""; $this.ForeColor = $script:theme.Text }
})
$txtSearch.Add_LostFocus({
    if ([string]::IsNullOrWhiteSpace($this.Text)) { $this.Text = "Search tools..."; $this.ForeColor = $script:theme.SubText }
})
...
$mainForm.Add_KeyDown({
    param($sender, $e)
    if ($e.Control -and $e.KeyCode -eq [System.Windows.Forms.Keys]::F) { $txtSearch.Focus(); $e.Handled = $true }
    if ($e.KeyCode -eq [System.Windows.Forms.Keys]::Escape) { $txtSearch.Text = "Search tools..."; $txtSearch.ForeColor = $script:theme.SubText; $cmbCategory.SelectedIndex = 0; Render-Tools; $e.Handled = $true }
})
...
$mainForm.Add_Resize({
    if ($script:contentPanel -and $script:contentPanel.Controls.Count -gt 0) {
        ...
        foreach ($control in $script:contentPanel.Controls) {
            if ($control -is [System.Windows.Forms.Panel]) { $control.Width = [math]::Min(320, $cardWidth) }
        }
    }
})
